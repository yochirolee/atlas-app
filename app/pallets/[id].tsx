...
export default function PalletScanner() {
  ...
  // Queries
  const { data: palletParcels, isLoading: loadingParcels } = useQuery({
    queryKey: ['pallet-parcels', palletId],
    queryFn: () => api.pallets.getParcelsByPalletId(palletId, 0, 100),
  });

  const scanMutation = useMutation({
    mutationFn: (scannedValue: string) => api.pallets.addParcelByScan(palletId, scannedValue),
    onSuccess: (data) => {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(console.error);
        queryClient.invalidateQueries({ queryKey: ['pallet-parcels', palletId] });
  ...
  const removeMutation = useMutation({
    mutationFn: (trackingNumber: string) => api.pallets.removeParcel(palletId, trackingNumber),
    onSuccess: () => {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(console.error);
        queryClient.invalidateQueries({ queryKey: ['pallet-parcels', palletId] });
  ...
  const parcelsList = palletParcels?.rows || [];
    ...
    <View style={styles.container}>
        ...
        <BlurView intensity={60} tint="dark" style={styles.topBarBlur}>
           <View style={styles.topBarRow}>
                ...
                <View style={styles.topBarCenter}>
                    <Text style={styles.topBarTitle}>Pallet #{palletId}</Text>
                    <View style={[styles.onlineBadge, { backgroundColor: Colors.purple }]}>
                        <Ionicons name="cloud-upload" size={12} color="#fff" />
                        <Text style={styles.onlineText}>Online Mode</Text>
                    </View>
                </View>
      ...
      <View style={[styles.bottomArea, { paddingBottom: insets.bottom + 16 }]}>
          ...
          <BlurView intensity={80} tint="dark" style={styles.statsBlur}>
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                ...
                <Text style={styles.statLab}>Pallet Count</Text>
              </View>
        ...
        <View style={styles.hintBox}>
            ...
            <Text style={styles.hintText}>Parcels are added instantly to the server.</Text>
        </View>
      </View>
        ...
        >
            <BlurView intensity={95} tint="light" style={styles.listBlur}>
                <View style={styles.listHeader}>
                    <Text style={styles.listTitle}>Pallet Parcels</Text>
                        ...
                        <View style={[styles.scanItem, { backgroundColor: Colors.elevated }]}>
                            ...
                            <TouchableOpacity 
                                onPress={() => {
                                    Alert.alert(
                                        'Remove Parcel',
                                        `Are you sure you want to remove ${item.hbl} from this pallet?`,
                                        [
