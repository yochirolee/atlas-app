...
export default function PackageDetail() {
  ...
  const { data, isLoading, error } = usePackageTracking(id as string);
  ...
  useEffect(() => {
    if (selectedHbl) {
      ...
      getLocationByHbl(selectedHbl).then(loc => {
        ...
        } else if (parcel && parcel.latitude && parcel.longitude) {
          setDeliveryLocation({
  ...
  }, [selectedHbl, parcel?.latitude, parcel?.longitude]);
  ...
  const parcel = data.parcels.find((p: any) => p.hbl === selectedHbl) || data.parcels[0];
  
  // Combine local and remote photos
  const allPhotos = [...(parcel.photos || []), ...localPhotos];
  
  // Map real status to a visual config
  const lastEvent = parcel.events?.[parcel.events.length - 1];
  const rawStatus = lastEvent?.location?.toUpperCase() || '';
  const statusKey = rawStatus.includes('RECIBIDO') ? 'DELIVERED' : 'IN_TRANSIT';
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle} numberOfLines={1}>{parcel?.description || 'Parcel Details'}</Text>
          <Text style={styles.headerTracking}>{parcel?.hbl || selectedHbl}</Text>
        </View>
      ...
      {data.parcels.length > 1 && (
        <View style={styles.parcelSelector}>
          <Text style={[styles.selectorLabel, { color: Colors.textSecondary }]}>Order contains {data.parcels.length} parcels:</Text>
      ...
      <View style={styles.statusHero}>
        <View style={[styles.statusIconBig, { backgroundColor: config.dimColor, borderColor: config.color + '40' }]}>
          <Ionicons name={config.icon as any} size={32} color={config.color} />
        </View>
        <Text style={[styles.statusBig, { color: config.color }]}>{lastEvent?.location || 'In Transit'}</Text>
        <View style={styles.routeHero}>
          ...
          <View style={[styles.routeHeroItem, { alignItems: 'flex-end' }]}>
            ...
            <Text style={styles.routeHeroValue}>
              {typeof data.city === 'object' ? data.city?.name : data.city}, {typeof data.province === 'object' ? data.province?.name : data.province}
            </Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.body}>
        ...
        {tab === 'timeline' && (
          <View style={styles.timeline}>
            {parcel.events.slice().reverse().map((event: any, idx: number, arr: any[]) => {
              ...
              return (
                <View key={idx} style={styles.timelineRow}>
                  ...
                  <View style={styles.timelineLeft}>
                    <Text style={[styles.timelineDateMonth, isFirst && { color: config.color }]}>{month}</Text>
                    <Text style={[styles.timelineDateDay, isFirst && { color: config.color }]}>{day}</Text>
                    <View style={[
                      styles.timelineDot,
                      isFirst
                        ? { backgroundColor: config.color, borderColor: config.color, shadowColor: config.color, shadowOpacity: 0.4, shadowRadius: 6, elevation: 4 }
                    ...
                    {!isLast && (
                      <View style={styles.timelineLineWrap}>
                        <View style={[styles.timelineLine, isFirst ? { backgroundColor: config.color + '30' } : { backgroundColor: Colors.cardBorder }]} />
                      </View>
                    )}
                  </View>
                  ...
                  <View style={[
                    ...
                    <Text style={[styles.timelineStatus, isFirst && { color: config.color }]}>{event.location}</Text>
        ...
        {tab === 'details' && (
          <View style={styles.detailsGrid}>
            {[
              { label: 'Tracking HBL', value: parcel.hbl, icon: 'qr-code-outline' },
              { label: 'Description', value: parcel.description, icon: 'document-text-outline' },
              { label: 'Weight', value: `${parcel.weight} kg`, icon: 'scale-outline' },
              { label: 'Invoice ID', value: `#${data.invoiceId}`, icon: 'receipt-outline' },
              { label: 'Origin', value: data.agency, icon: 'business-outline' },
              { 
                label: 'Destination', 
          ...
          <View style={styles.proofWrap}>
            ...
            {allPhotos.length > 0 ? (
              ...
              <View style={styles.emptyProof}>
                ...
                <Text style={styles.emptyProofSub}>Proof of delivery will appear here once the package is delivered.</Text>
              </View>
            )}
          </View>
        )}
