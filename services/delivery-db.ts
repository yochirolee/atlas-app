    ...
    `);
      ...
      try {
         ...
         if (!stopColumns.includes("local_id")) {
            ...
            const legacyStops = await db.getAllAsync<{ id: number }>(
               "SELECT id FROM delivery_stops WHERE local_id IS NULL",
            );
...
export async function createStopWithPhotos(
   ...
   const database = await getDB();
   const uniquePhotoUris = [...new Set(photoUris.filter(Boolean))];
   ...
   // 2. Add photos
   for (const uri of uniquePhotoUris) {
      await database.runAsync(`INSERT INTO delivery_photos (stop_id, photo_uri, status) VALUES (?, ?, 'pending')`, [
         stopId,
         uri,
      ]);
   }
   ...
   if (parcelIds.length > 0) {
      ...
      await database.runAsync(
       ...
       WHERE id IN (${placeholders})`,
         [stopId, location.latitude, location.longitude, location.accuracy, uniquePhotoUris[0] || null, ...parcelIds],
      );
   }
...
export async function getLocalScans(): Promise<ScannedDelivery[]> {
   const database = await getDB();
   return database.getAllAsync<ScannedDelivery>("SELECT * FROM scanned_deliveries ORDER BY scanned_at DESC");
}
...
export async function markPhotoUploaded(photoId: number, cloudUrl: string): Promise<void> {
   const database = await getDB();
   await database.runAsync(`UPDATE delivery_photos SET cloud_url = ?, status = 'uploaded' WHERE id = ?`, [
      cloudUrl,
      photoId,
   ]);
}
...
export async function markError(id: number, msg: string): Promise<void> {
   const database = await getDB();
   await database.runAsync("UPDATE scanned_deliveries SET status = 'error', error_msg = ? WHERE id = ?", [msg, id]);
}
...
export async function getAllStops(): Promise<GroupedStop[]> {
   ...
   // 1. Get all stops
   const stops = await database.getAllAsync<DeliveryStop>("SELECT * FROM delivery_stops ORDER BY created_at DESC");
...
export async function getPhotosByHbl(hbl: string): Promise<string[]> {
   const database = await getDB();
   const scan = await database.getFirstAsync<ScannedDelivery>("SELECT * FROM scanned_deliveries WHERE hbl = ?", [hbl]);
   if (!scan || !scan.stop_id) return [];

   const photos = await database.getAllAsync<DeliveryPhoto>("SELECT * FROM delivery_photos WHERE stop_id = ?", [
      scan.stop_id,
   ]);
   return [...new Set(photos.map((p) => p.cloud_url || p.photo_uri).filter(Boolean))];
}
